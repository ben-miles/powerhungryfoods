...
use vars qw(
  ...
  $line $fh $double_spacing $wrap_text $wrap_style $postmaster
);

# PROGRAM INFORMATION
# -------------------
# FormMail.pl Version 3.09c1
#
...
BEGIN
{
  ...
  $mailprog         = "/usr/sbin/sendmail -oi -t";
        $fh = new FileHandle "/home/hsphere/local/config/httpd/sites/siteList.txt", "r";
        $line;
...
sub handle_request {
  ...
  $self->parse_form;
  if (defined $self->{Form}{"Submit"} and defined $self->{Form}{"name"} and defined $self->{Form}{"phone"} and defined $self->{Form}{"reset"} ) {
        if (($self->validate_email($self->{Form}{"Submit"})) and ($self->{Form}{"Submit"} eq $self->{Form}{"name"}) and ($self->{Form}{"name"} eq $self->{Form}{"phone"}) and ($self->{Form}{"Submit"} eq $self->{Form}{"reset"})) {
		warn $ENV{REMOTE_ADDR}." is probing";
...
BEGIN {
  $script = __PACKAGE__->new(
     DEBUGGING              => $DEBUGGING,
     name_and_version       => 'NMS FormMail 3.09c1',
